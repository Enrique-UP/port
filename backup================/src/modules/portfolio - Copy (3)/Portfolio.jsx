...
const Temp = ({ date, site, link, about, noteClass, img, onClick }) => {
              ...
              <div className="port">
                {PortfolioData.map((section, i) => (
                  <div className="portArea" key={i}>
                    ...
                    {section.items.map(
                      ({ date, name, domain, link, note }, j) => {
                        const site = `${name}.${domain}`;
                        const about = `${name}_text`;

                        // ✅ note class generate
                        const noteClass = note ? `notes ${name}_${domain}_note` : "";
                        ...
                        const index =
                          PortfolioData.slice(0, i).flatMap((s) => s.items).length + j;
                        ...
                        return (
                          <Temp
                            ...
                            link={fullLink}
                            about={about}
                            noteClass={noteClass}   // 👈 pass class
